...
namespace GetStoreApp.WindowsAPI.PInvoke.Kernel32
{
    ...
    public static partial class Kernel32Library
    {
        ...
        /// 指向 SECURITY_ATTRIBUTES 结构的指针，该结构确定返回的句柄是否可以由子进程继承。 如果 <param name="lpPipeAttributes"> 为 NULL，则无法继承句柄。
        ...
        [LibraryImport(Kernel32, EntryPoint = "CreatePipe", SetLastError = false), PreserveSig]
        [return: MarshalAs(UnmanagedType.Bool)]
        public static unsafe partial bool CreatePipe(out nint hReadPipe, out nint hWritePipe, SECURITY_ATTRIBUTES* lpPipeAttributes, uint nSize);
        ...
        /// 指向 STARTUPINFO 或 STARTUPINFOEX 结构的指针。若要设置扩展属性，请使用 STARTUPINFOEX 结构并在 <param name="dwCreationFlags"> 参数中指定 EXTENDED_STARTUPINFO_PRESENT。当不再需要 STARTUPINFO 或 STARTUPINFOEX 中的句柄时，必须使用 CloseHandle 关闭它们。
        ...
        /// 指向使用同步 <param name="hFile"> 参数时接收读取的字节数的变量的指针。 ReadFile 将此值设置为零，然后再执行任何工作或错误检查。
