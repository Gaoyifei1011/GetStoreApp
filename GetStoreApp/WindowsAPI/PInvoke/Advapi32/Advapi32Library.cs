...
namespace GetStoreApp.WindowsAPI.PInvoke.Advapi32
{
    public static class Advapi32Library
    {
        ...
        /// <returns>如果函数成功，则返回值为 ERROR_SUCCESS。如果函数失败，则返回值为 Winerror.h 中定义的非零错误代码。</returns>
        [DllImport(Advapi32, CharSet = CharSet.Unicode, EntryPoint = "RegOpenKeyEx", SetLastError = true)]
        public static extern int RegOpenKeyEx(IntPtr hKey, string lpSubKey, int ulOptions, RegistryAccessRights samDesired, ref IntPtr phkResult);
        ...
        /// </returns>
        [DllImport(Advapi32, CharSet = CharSet.Unicode, EntryPoint = "RegQueryValueEx", SetLastError = true)]
