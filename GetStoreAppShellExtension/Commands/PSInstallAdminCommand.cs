...
namespace GetStoreAppShellExtension.Commands
{
    ...
    public partial class PSInstallAdminCommand : IExplorerCommand
    {
        ...
        {
            pguidCommandName = typeof(PSInstallAdminCommand).GUID;
            return 0;
        }
        ...
        public int Invoke(IShellItemArray psiItemArray, nint pbc)
        {
            if (psiItemArray is not null && psiItemArray.GetCount(out uint count) is 0 && count >= 1 && psiItemArray.GetItemAt(0, out IShellItem shellItem) is 0)
            {
                ...
                Shell32Library.ShellExecute(nint.Zero, "runas", "PowerShell.exe", arguments, Path.GetDirectoryName(filePath), WindowShowStyle.SW_SHOWNORMAL);
            }
            return 0;
        }
