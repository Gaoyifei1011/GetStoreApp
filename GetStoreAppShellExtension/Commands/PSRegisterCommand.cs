...
namespace GetStoreAppShellExtension.Commands
{
    ...
    public partial class PSRegisterCommand : IExplorerCommand
    {
        ...
        {
            pguidCommandName = typeof(PSRegisterCommand).GUID;
            return 0;
        }
        ...
        public int GetState(IShellItemArray psiItemArray, bool fOkToBeSlow, out EXPCMDSTATE pCmdState)
        {
            if (psiItemArray is not null && psiItemArray.GetCount(out uint count) is 0 && count >= 1 && psiItemArray.GetItemAt(0, out IShellItem shellItem) is 0)
            {
                ...
                pCmdState = string.Equals(Path.GetFileName(filePath), "AppxManifest.xml", StringComparison.OrdinalIgnoreCase) ? EXPCMDSTATE.ECS_ENABLED : EXPCMDSTATE.ECS_HIDDEN;
            }
            else
            {
        ...
        public int Invoke(IShellItemArray psiItemArray, nint pbc)
        {
            if (psiItemArray is not null && psiItemArray.GetCount(out uint count) is 0 && count >= 1 && psiItemArray.GetItemAt(0, out IShellItem shellItem) is 0)
            {
                ...
                Shell32Library.ShellExecute(nint.Zero, "open", "PowerShell.exe", arguments, Path.GetDirectoryName(filePath), WindowShowStyle.SW_SHOWNORMAL);
            }
            return 0;
        }
