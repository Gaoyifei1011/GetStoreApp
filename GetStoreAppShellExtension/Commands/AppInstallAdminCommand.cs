...
namespace GetStoreAppShellExtension.Commands
{
    ...
    public partial class AppInstallAdminCommand : IExplorerCommand
    {
        ...
        public int Invoke(IShellItemArray psiItemArray, IntPtr pbc)
        {
            ...
            {
                shellItem.GetDisplayName(SIGDN.SIGDN_FILESYSPATH, out string filePath);
                Shell32Library.ShellExecute(IntPtr.Zero, "runas", "GetStoreAppInstaller.exe", string.Join(' ', filePath, "--elevated"), Path.GetDirectoryName(filePath), WindowShowStyle.SW_SHOWNORMAL);
            }
            return 0;
        }
