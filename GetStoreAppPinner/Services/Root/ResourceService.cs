    ...
    public static class ResourceService
    {
        ...
        public static string GetLocalized(string resource)
        {
            if (isInitialized)
            {
                ...
                {
                    LogService.WriteLog(LoggingLevel.Error, nameof(GetStoreAppPinner), nameof(ResourceService), nameof(GetLocalized), 1, e);
                    return resource;
                }
            }
            else
            {
                LogService.WriteLog(LoggingLevel.Error, nameof(GetStoreAppPinner), nameof(ResourceService), nameof(GetLocalized), 2, new Exception());
                return resource;
            }
        }
    }
