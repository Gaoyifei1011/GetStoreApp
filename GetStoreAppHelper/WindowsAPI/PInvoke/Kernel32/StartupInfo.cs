...
namespace GetStoreAppHelper.WindowsAPI.PInvoke.Kernel32
{
    ...
    public unsafe partial struct STARTUPINFO
    {
        ...
        /// <summary>
        /// 如果 <see cref="dwFlags"/> 指定 <see cref="StartupInfoFlags.STARTF_USEPOSITION">，则如果创建新窗口（以像素为单位），则此成员是窗口左上角的 x 偏移量。 否则，将忽略此成员。
        ...
        /// <summary>
        /// 如果 <see cref="dwFlags"/> 指定 <see cref="StartupInfoFlags.STARTF_USEPOSITION">，则如果创建新窗口（以像素为单位），则此成员是窗口左上角的 y 偏移量。 否则，将忽略此成员。
        ...
        /// <summary>
        /// 如果 <see cref="dwFlags"/> 指定 <see cref="StartupInfoFlags.STARTF_USESIZE">，则如果创建新窗口（以像素为单位），则此成员是窗口的宽度。 否则，将忽略此成员。
        ...
        /// <summary>
        /// 如果 <see cref="dwFlags"/> 指定 <see cref="StartupInfoFlags.STARTF_USESIZE">，则如果创建新窗口（以像素为单位），则此成员是窗口的高度。 否则，将忽略此成员。
        ...
        /// <summary>
        /// 如果 <see cref="dwFlags"/> 指定 <see cref="StartupInfoFlags.STARTF_USECOUNTCHARS">，如果在控制台进程中创建了一个新的控制台窗口，则此成员以字符列指定屏幕缓冲区宽度。 否则，将忽略此成员。
        /// </summary>
        public int dwXCountChars;

        /// <summary>
        /// 如果 <see cref="dwFlags"/> 指定 <see cref="StartupInfoFlags.STARTF_USECOUNTCHARS">，如果在控制台进程中创建了一个新的控制台窗口，则此成员在字符行中指定屏幕缓冲区高度。 否则，将忽略此成员。
        /// </summary>
        public int dwYCountChars;

        /// <summary>
        /// 如果 <see cref="dwFlags"/> 指定 <see cref="StartupInfoFlags.STARTF_USEFILLATTRIBUTE">，则如果控制台应用程序中创建了新的控制台窗口，则此成员是初始文本和背景色。 否则，将忽略此成员。
        ...
        /// <summary>
        /// If <see cref="dwFlags"/> specifies <see cref="StartupInfoFlags.STARTF_USESHOWWINDOW"/>, 则此成员可以是在 <see cref="User32Library.ShowWindow"> 函数的 nCmdShow 参数中指定的任何值，但 <see cref="WindowShowStyle.SW_SHOWDEFAULT"> 除外。 否则，将忽略此成员。
        ...
        /// <summary>
        /// 如果 <see cref="dwFlags"/> 指定 <see cref="StartupInfoFlags.STARTF_USESTDHANDLES"/>, 则此成员是进程的标准输入句柄。 如果未指定 <see cref="StartupInfoFlags.STARTF_USESTDHANDLES"/>，则标准输入的默认值为键盘缓冲区
        /// 如果 <see cref="dwFlags"/> 指定 <see cref="StartupInfoFlags.STARTF_USEHOTKEY"/>, 则此成员指定一个热键值，该值作为 <see cref="WindowMessage.WM_SETHOTKEY"> 消息的 wParam 参数发送到拥有该过程的应用程序创建的第一个符合条件的顶级窗口。如果使用 <see cref="WindowStyles.WS_POPUP"> 窗口样式创建窗口，则它不符合条件，除非还设置了 <see cref="WindowStylesEx.WS_EX_APPWINDOW"> 扩展窗口样式。 有关详细信息，请参阅 <see cref="User32Library.CreateWindowEx">。
        /// 否则，将忽略此成员。
        /// </summary>
        public IntPtr hStdInput;

        /// <summary>
        /// 如果 <see cref="dwFlags"/> 指定 <see cref="StartupInfoFlags.STARTF_USESTDHANDLES"/>, 则此成员是进程的标准输出句柄。 否则，将忽略此成员，标准输出的默认值为控制台窗口的缓冲区。
        ...
        /// <summary>
        /// 如果 <see cref="dwFlags"/> 指定 <see cref="StartupInfoFlags.STARTF_USESTDHANDLES"/>, 则此成员是进程的标准错误句柄。 否则，将忽略此成员，标准错误的默认值为控制台窗口的缓冲区。
        /// </summary>
