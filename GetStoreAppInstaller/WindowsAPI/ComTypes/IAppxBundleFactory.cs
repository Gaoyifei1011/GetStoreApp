...
namespace GetStoreAppInstaller.WindowsAPI.ComTypes
{
    ...
    public partial interface IAppxBundleFactory
    {
        ...
        /// 创建可向其添加应用包的仅写捆绑包对象。
        /// </summary>
        /// <param name="inputStream">接收序列化包数据的输出流。 流必须至少支持 Write 方法。</param>
        ...
        /// </param>
        /// <param name="bundleReader">此方法创建的捆绑编写器。</param>
        /// <returns>如果方法成功，则返回 S_OK。 否则，它将返回一个错误代码，该错误代码包括但不限于下表中的错误代码。</returns>
        [PreserveSig]
        int CreateBundleWriter([MarshalAs(UnmanagedType.Interface)] IStream inputStream, ulong bundleVersion, [MarshalAs(UnmanagedType.Interface)] out IAppxBundleReader bundleReader);
        ...
        /// <returns>如果该方法成功，则返回 S_OK。 否则，它将返回一个错误代码，该代码包括但不限于下表中的错误代码。</returns>
        [PreserveSig]
